     ...
     if(raw.size() > 2){
         ...
         for(int i=1; i<vsize+1; i++){
            ...
            for(int j=1; j<raw.size()-2; j++){
                if( now[1]<=j ){
                    if( wi[j-now[1]][i-1]+val > wi[j][i-1] ){
                        if( now[0]>0 ){
         ...
         for(int i=0; i<raw.size()-2; i++){
            ...
            if( thisw==3 ){
                ...
                while(thisi != vsize-1 && thisw != 0){
                    now = newv[thisi+1];
                    if( now[0]>0 ){
                        if( map1[now[0]].size()!=2 || sizeone[i]!=0 ){
                        ...
                        if( lastmap.find((-1)*now[0]) != lastmap.end() ){
                            ...
                            for(; tmp!=lastmap.end(); tmp++){
                                ...
                                else if( (tmp->first)>0 && (tmp->second)[0]==2 ) break;
                            }
                            if( tmp!=lastmap.end() ){
